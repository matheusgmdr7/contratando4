    ...
    try {
      ...
      if (!templateId && templateTitulo) {
        ...
        if (modelo) {
          ...
          const { error } = await supabaseClient.from("propostas").update({ modelo_id: modelo.id }).eq("id", propostaId)
      ...
      const { data: dependentesData, error: dependentesError } = await supabaseClient
        .from("dependentes")
        .select("*")
      ...
      const { data: questionarioData, error: questionarioError } = await supabaseClient
